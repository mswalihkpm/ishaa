...
const BookListItem = ({ book, categories, onEdit, onDelete, onManageBorrow }) => {
  ...
  const getCoverImageSrc = () => {
    ...
    if (book.coverImage) {
      // If it's a File object with a preview URL, make one.
      if (book.coverImage instanceof File) {
        return URL.createObjectURL(book.coverImage);
      }

      // If it's already a base64 string
      if (typeof book.coverImage === 'string' && book.coverImage.startsWith('data:image')) {
      ...
      <div className="flex justify-between items-start">
        <div className="flex items-start space-x-3">
          {coverSrc ? (
            <img
              src={coverSrc}
              alt={book.name}
              className="h-16 w-12 object-cover rounded-sm border border-border/50"
            />
          ) : (
            <div className="h-16 w-12 bg-muted/50 dark:bg-muted/40 rounded-sm flex items-center justify-center">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              By {book.author || 'N/A'} | BN: {book.bookNumber}
            </p>
            <p className="text-xs text-muted-foreground">
              Category:{' '}
        ...
        <div className="flex flex-col sm:flex-row space-y-1 sm:space-y-0 sm:space-x-1 shrink-0">
          ...
          >
            ...
            {book.isBorrowed ? 'Manage Return' : 'Manage Borrow'}
          </Button>
          <Button
            variant="ghost"
            size="icon"
